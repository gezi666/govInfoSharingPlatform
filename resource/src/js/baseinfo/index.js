...
function statisticItem(id){
    if(id){
        switch(id){
            case '1':
                statisticalColumn('5', '基础信息')
                break
            case '2':
                statisticalColumn('6', '政务主题')
                break
            case '3':
                statisticalColumn('7', '部门资源')
                break
            case '307':
                statisticalColumn('8', '地方资源')
                break
        }
    }
...
function getListData(pageIndex, loadPaga) {
    ...
    api.get('/sourceinfo/find', params)
        .done(function (result) {
            result.isLogin =  tool.getCookie('currUser');
    ...
    api.get("/direct/find?code=" + id).then(function (result) {
        ...
        if (type == 1) {
            ...
            $(result.data).each(function (index, item) {
                html += '<div><a class="clickable2" onclick="treeClick(2,' + item.code + ',this)"><em></em>' + '<span>' + item.name + '</span></span>' + '<i class="icon iconfont icon-arrow-t"></i></a></div>'
            });
            html += "</div>"
        }
        else {
            ...
            $(result.data).each(function (index, item) {
                html += '<a onclick="treeClick(3,' + item.code + ',this)">' + '<span>' + item.name + '</span>' + '</a>'
            });
            html += "</div>";
        }
